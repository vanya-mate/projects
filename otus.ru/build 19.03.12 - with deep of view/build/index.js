...
// if cookie empty (fiest visit) || != 1 (referrer == otus) => oldGuest = 0 || if oldGuest = 1 => oldGuest = 1;
...
if (document.referrer.search('://otus.ru') == '-1' && Kameleoon.Utils.readLocalData('oldGuest', 'COOKIE') != 1 || Kameleoon.Utils.readLocalData('deepView', 'COOKIE') != window.location.href) {
  ...
  if (document.referrer.search('://otus.ru') == '-1') {
    Kameleoon.Utils.createLocalData('oldGuest', 1, 1 / 24 / 60, '', 'COOKIE'); // if referrer != otus => oldGuest = 1;
  } else {
    Kameleoon.Utils.createLocalData('oldGuest', 0, 1 / 24 / 60, '', 'COOKIE'); // if referrer == otus => oldGuest = 0;
  }
