  ...
  if(splitHref[1] == 'kangoo.html') {
    ...
    modelsSplit.forEach((elem) => {
      if(elem === splitHref[3].split('.html')[0]) {
        ...
        if(elem === 'twizy') {
          if(splitHref[4]) {
            console.log('its twizy-cargo');
            // .. adding in cokkie twizy-cargo
            toJson = JSON.stringify({type: splitHref[2], model: 'twizy-cargo'});
          } else {
            console.log('its twizy');
            // .. adding in cookie twizy
          }
        }
        if(elem === 'captur') {
          if(window.location.hash == '#initiale-paris') {
            console.log('its captur+');
            // .. adding in cookie captir+
            toJson = JSON.stringify({type: splitHref[2], model: 'captur-ip'});
          } else {
            console.log('its captur');
            // .. adding in cookie captur
          }
        }
        Kameleoon.Utils.createLocalData('lastViewedCar', toJson, 365, '', 'COOKIE');
      }
...
Kameleoon.API.runWhenElementPresent('#closedPopInKam', () => {
