...
var hidePopIn = function hidePopIn() {
  document.body.removeChild(popInElement);
};

var showPopIn = function showPopIn() {
  document.body.insertAdjacentElement('afterBegin', popInElement);
};
  ...
  if (splitHref[1] == 'kangoo.html') {
    ...
    modelsSplit.forEach(function (elem) {
      if (elem === splitHref[3].split('.html')[0]) {
        console.log("type: ".concat(splitHref[2], ", model: ").concat(elem)); // .. adding in cookie
        ...
        if (elem === 'twizy') {
          if (splitHref[4]) {
            console.log('its twizy-cargo'); // .. adding in cokkie twizy-cargo
            ...
            console.log('its twizy'); // .. adding in cookie twizy
          }
        }

        if (elem === 'captur') {
          if (window.location.hash == '#initiale-paris') {
            console.log('its captur+'); // .. adding in cookie captir+
            ...
            console.log('its captur'); // .. adding in cookie captur
          }
        }
...
lastViewedCar();
Kameleoon.API.runWhenElementPresent('#closedPopInKam', function () {
  document.getElementById('closedPopInKam').addEventListener('click', hidePopIn);
});
